	...
	<section id="adminmachinegroups">
		<div style="overflow: hidden;">
			<table class="table table-striped">
				...
				<tr id="article_<?php echo $guide['Article']['id'];?>">
					<td width=30%>
						<h5>
							<a href="<?php echo $this->Html->url(array("controller" => "dashboard","action" => "article", "?"=>array("article_id" => $guide['Article']['id'])));?>">
								<?php echo $guide['Article']['title'];?>
							</a>
						</h5>
						<h6>-来自<?php echo $guide['Brand']['name'];?></h6>
					</td>
					<td width=50%>
						<?php echo $this->StringUtil->sysSubStr($guide['Article']['content'], 200);?>
					</td>
					<td width=20%>
						<ul class="unstyled">
							<li>
								<a href="javascript:void(0);" class="btn-mini btn-danger delArticle" title="危险操作，请谨慎" data-aname="<?php echo $guide['Article']['title'];?>" data-aid="<?php echo $guide['Article']['id'];?>">
									删除该导购
								</a>
								&nbsp;
								<a href="<?php echo $this->Html->url(array("controller" => "dashboard","action" => "editArticle", "?"=>array("article_id" => $guide['Article']['id'])));?>" class="btn-mini btn-warning">
									编辑该导购
								</a>
							</li>
						</ul>
			...
			<div class="pagination pagination-right">
				...
				<?php
					$this->Paginator->options(array(
						'url'=> array('controller'=>'dashboard', 'action'=>'guides')
					));
				?>
				<?php
					if(count($guides) >= 1){
